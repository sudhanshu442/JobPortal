...
const JobDescription = () => {
    ...
    const jobApplyHandler=async()=>{
        try {
            ...
            if(res.data.success){
                setIsApplied(true); //update the local job
...
toast.error(error.response?.data?.message || "Something went wrong");
    ...
    useEffect(() => {
            ...
            try {
                ...
                setIsApplied(res.data.job.applications.some(application=>application.applicant===user?._id));  //Ensure state in sync daata
                }
            } catch (error) {
                console.log(error);
            }
    ...
    return (
        <div className='max-w-7xl mx-auto my-10'>
         <div className="flex item-center justify-between">
            ...
            <Button onClick={isApplied? null:jobApplyHandler} disabled={isApplied} className={`rounded-lg ${isApplied?'bg-gray-600 cursor-not-allowed':'bg-[7289b7] hover:bg-[#5f32ad]'} `}>{isApplied?"Already Applied" : "Apply Now"}</Button>
         </div>
